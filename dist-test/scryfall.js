...
import { isBiType, manaLetters, manaLetterToType as manaLetterToTypeMap } from "../types/mana";
class RequestQueue {
    queue = [];
    processing = false;
    lastRequestTime = 0;
    minDelay = 200; // Aumentado para 200ms para ser mais conservador
    async add(fn) {
...
async function fetchCachedJson(url, retryCount = 0, options = {}) {
...
export function manaLetterToType(manaLetter) {
    if (manaLetters.includes(manaLetter)) {
        return manaLetterToTypeMap[manaLetter];
    }
    else {
        return [...new Array(parseInt(manaLetter) || 0)].map(() => "colorless");
    }
}
const MTG_TERMS_PT = {
...
const ALL_KEYWORDS_EN = Object.keys(MTG_TERMS_PT)
    .filter(en => BOLD_KEYWORDS.includes(en))
    .sort((a, b) => b.length - a.length);
export function enrichOracleText(text, lang = "en") {
    if (!text)
        return text;
...
let customDictionaryCache = null;
export async function getCustomDictionary() {
    if (customDictionaryCache)
    ...
    try {
        const dict = await idbGet("mtg-custom-dict");
        customDictionaryCache = dict || {};
        return customDictionaryCache;
    }
    catch {
...
export async function saveToCustomDictionary(enText, ptText) {
    ...
    try {
        ...
        if (response.ok)
            return; // Se salvou via API local, sucesso!
    }
    catch (e) {
...
function prepareMtgText(text, lang, customDict) {
    if (lang !== 'pt')
        return { preparedText: text, placeholders: [] };
...
function restoreMtgText(translatedText, _placeholders) {
...
async function translateGoogle(text, targetLang) {
    ...
    try {
        const dict = await getCustomDictionary();
        const { preparedText, placeholders } = prepareMtgText(text, targetLang, dict);
        ...
        let translatedText = json?.[0]?.map((x) => x?.[0] || '')?.join('') || preparedText;
...
export async function processCorrection(originalEn, editedPt, targetLang = "pt") {
    if (!originalEn || !editedPt)
        return;
    const googleTranslated = await translateGoogle(originalEn, targetLang);
    const enLines = originalEn.split('\n');
    const editedLines = editedPt.split('\n');
    const googleLines = googleTranslated.split('\n');
    if (enLines.length === editedLines.length && editedLines.length === googleLines.length) {
        for (let i = 0; i < enLines.length; i++) {
            const enLine = enLines[i].trim();
            const editedLine = editedLines[i].trim();
            const googleLine = googleLines[i].trim();
            if (enLine && editedLine && editedLine !== googleLine && enLine !== editedLine) {
                await saveToCustomDictionary(enLine, editedLine);
            }
        }
    }
    else {
        if (originalEn !== editedPt && editedPt !== googleTranslated) {
            await saveToCustomDictionary(originalEn.trim(), editedPt.trim());
        }
    }
}
function needScan(scryfallResult) {
...
function getCardScanUrl(scryfallResult, { ifNecessary }) {
    ...
    if ('image_uris' in scryfallResult) {
        ...
        uris = scryfallResult['card_faces'].find((f) => 'image_uris' in f)?.['image_uris'];
    }
    return uris['large'] ?? uris['normal'] ?? uris['small'];
}
export function mapScryfallToCard(scryfallCard, lang) {
    const enCardFaceInfo = scryfallCard.layout === 'transform' && scryfallCard.card_faces?.length === 2
        ? scryfallCard.card_faces[0]
        : scryfallCard;
    const colorsToUse = enCardFaceInfo["type_line"].toLowerCase().includes("land")
    ...
    const card = {
        ...
        aspect: {
            ...
            color: parseCardColor(manaTypes, enCardFaceInfo["type_line"].toLowerCase().includes("artifact") &&
                !enCardFaceInfo["type_line"].toLowerCase().includes("vehicle"), manaCost
                .filter((type) => type != "colorless" && type != "x")
                .every(isBiType)),
            legendary: scryfallCard["frame_effects"]?.includes("legendary") ||
        ...
        ...(enCardFaceInfo["type_line"].toLowerCase().includes("planeswalker")
            ? { category: "Planeswalker", loyalty: enCardFaceInfo["loyalty"] }
            : { category: "Regular" }),
        overrideWithScanUrl,
    };
    if (scryfallCard.layout === 'transform' && scryfallCard.card_faces?.length === 2) {
        ...
        card.verso = {
            ...
            aspect: {
                ...
                color: parseCardColor(manaTypes, enReverseFaceInfo["type_line"].toLowerCase().includes("artifact") &&
                    !enReverseFaceInfo["type_line"].toLowerCase().includes("vehicle"), manaCost
                    .filter((type) => type != "colorless" && type != "x")
                    .every(isBiType)),
                legendary: scryfallCard["frame_effects"]?.includes("legendary") ||
            ...
            ...(enReverseFaceInfo["type_line"].toLowerCase().includes("planeswalker")
                ? { category: "Planeswalker", loyalty: enReverseFaceInfo["loyalty"] }
                : { category: "Regular" }),
            overrideWithScanUrl,
        };
    }
...
export async function fetchCardsCollection(names) {
    if (names.length === 0)
        return [];
...
export async function fetchCardsBulk(names, lang = "en", onProgress) {
    const chunks = [];
    ...
    for (const chunk of chunks) {
        ...
        for (let i = 0; i < chunk.length; i++) {
            ...
            if (lang !== "en" && scryfallCard.lang === "en") {
                ...
                card.typeText = await translateGoogle(lang === "pt" ? card.typeText.replace(/Basic Land/gi, "[[Terreno Básico]]") : card.typeText, lang);
                card.oracleText = enrichOracleText(await translateGoogle(card.oracleText, lang), lang);
                if (card.flavorText)
                    card.flavorText = await translateGoogle(card.flavorText, lang);
                if (card.verso && typeof card.verso !== "string") {
                    card.verso.title = await translateGoogle(card.verso.title, lang);
                    card.verso.typeText = await translateGoogle(lang === "pt" ? card.verso.typeText.replace(/Basic Land/gi, "[[Terreno Básico]]") : card.verso.typeText, lang);
                    card.verso.oracleText = enrichOracleText(await translateGoogle(card.verso.oracleText, lang), lang);
                    if (card.verso.flavorText)
                        card.verso.flavorText = await translateGoogle(card.verso.flavorText, lang);
                }
            }
            allCards.push(card);
        }
...
export async function fetchCard(title, lang = "en", variant = 0) {
    let frCards = await fetchCachedJson(`https://api.scryfall.com/cards/search/?q=((!"${title}" lang:${lang}) or ("${title}" t:token)) order:released direction:asc`);
    let enCards = await fetchCachedJson(`https://api.scryfall.com/cards/search/?q=((!"${title}") or ("${title}" t:token)) order:released direction:asc`);
    ...
    const findMatch = (cards) => {
        ...
        const exact = cards.data.find((c) => c.name.toLowerCase() === title.toLowerCase());
        if (exact)
            return exact;
        // Fallback para o primeiro que inclui o título
        const partial = cards.data.find((c) => c.name.toLowerCase().includes(title.toLowerCase()));
        if (partial)
            return partial;
    ...
    const needsTranslation = lang !== "en" && (frCardsStatus === 404 ||
        fr["lang"] === "en" ||
        !hasPrintedText);
    ...
    if (needsTranslation) {
        ...
        card.typeText = await translateGoogle(targetLang === "pt" ? card.typeText.replace(/Basic Land/gi, "[[Terreno Básico]]") : card.typeText, targetLang);
        ...
        if (card.verso && typeof card.verso !== "string") {
            ...
            card.verso.typeText = await translateGoogle(targetLang === "pt" ? card.verso.typeText.replace(/Basic Land/gi, "[[Terreno Básico]]") : card.verso.typeText, targetLang);
    ...
    let selectedVariant = variants.length > 0 ? (variants[variant % variants.length] ?? {}) : {};
    if (lang !== "en" && selectedVariant.typeText) {
        selectedVariant = {
            ...selectedVariant,
            typeText: await translateGoogle(targetLang === "pt" ? selectedVariant.typeText.replace(/Basic Land/gi, "[[Terreno Básico]]") : selectedVariant.typeText, targetLang),
            oracleText: enrichOracleText(await translateGoogle(selectedVariant.oracleText ?? "", targetLang), lang),
...
export async function fetchVariants(title) {
    const response = await fetchCachedJson(`https://api.scryfall.com/cards/search/?q=!"${title}" unique:art prefer:newest`);
    const variants = (response.data || [])
        ...
        if (card["type_line"]?.toLowerCase().includes("token")) {
            const manaTypes = (card["colors"] ?? card["color_identity"] ?? []).flatMap(manaLetterToType);
            ...
            partial = {
                ...
                aspect: {
                    ...
                    color: parseCardColor(manaTypes, card["type_line"].toLowerCase().includes("artifact") &&
                        !card["type_line"].toLowerCase().includes("vehicle"), manaCost
                        .filter((type) => type != "colorless" && type != "x")
                        .every(isBiType)),
                    legendary: card["frame_effects"]?.includes("legendary") ||
...
export async function fetchCardType(name) {
    const response = await fetchCachedJson(`https://api.scryfall.com/cards/search/?q=!"${name}"`);
    const [card] = response.data ?? [];
    return card?.["type_line"] ?? "";
}
export async function searchCard(search) {
    if (search.length < 3)
        return [];
    const response = await fetchCachedJson(`https://api.scryfall.com/cards/search/?q=${encodeURIComponent(search)}`);
    if (response.status === 404)
        return [];
    if (response.status >= 500)
        throw new Error("Erro na API do Scryfall");
    const result = response.data;
    const deduped = result.filter((card, index) => result.findIndex((c) => c.name == card.name) == index);
    return deduped;
}
